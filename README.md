...
- **Saved spots**: record any cursor position and warp straight back to it with `Cmd + <key>` (works in red mode *and* the grid layer). Arm recording with `Cmd + Shift + S`, then press `Cmd + <key>` to store the live position into that slot. In the grid layer each saved spot shows its warp combo (e.g. `⌘J`) on a small **blue badge** under the cell it falls in.
...
| **Warp to saved spot** | `Cmd + <key>` (works in red mode and the grid layer) |
| **Record a saved spot** | `Cmd + Shift + S` to arm, then `Cmd + <key>` to store the current cursor position |
| **Panic exit** | `Esc` (auto-commits drag if in progress; cancels an armed spot-record) |
...
The cell the cursor currently sits in is flooded in pale neon yellow, framed with a **hot-pink 3px outline**, and has its label drawn on a **hot-pink pill** — all three track the cursor as you move, so you always know where you are ("I'm in `BC`, I want `FJ`"). The point is coarse-jump first (kill the long travel), then either type the next cell or step with arrows — much faster than crawling the cursor across the whole screen.
...
### Saved spots (record a position, warp back to it)

Saved spots let you bookmark cursor positions and jump straight back with one shortcut — handy for spots you return to constantly (a toolbar button, a dock target, a chat input). They work in **both red mode and the grid layer**.

- **Warp**: `Cmd + <key>` (default modifier is `Cmd`) warps the cursor to the recorded position for that key. Movement keys are excluded (so `Cmd + arrow` stays scroll), and `Cmd + <key>` is only consumed when that slot exists — so `Cmd + C`, `Cmd + V`, etc. still pass through to the foreground app.
- **Record**: press the arm combo `Cmd + Shift + S` (the red badge signals "armed"), then press `Cmd + <key>` within the arm window (default 8s) to store the **current** cursor position into that slot. Recording a key that's already used overwrites it. `Esc` cancels an armed record.
- Recorded spots persist to `~/.mMouse.json` (`savedSpots.spots`) automatically, so they survive restarts.
- In the grid layer, each saved spot that falls on the current display shows its warp combo (e.g. `⌘J`) on a small **blue badge** under the cell it lands in, so you can see your bookmarks at a glance.

All four pieces — warp modifier, record combo, arm window, and the stored spots — are configurable under `savedSpots` (see [Config](#config)).
...
| `savedSpots.warpModifier` | Modifier held with a slot key to warp to a recorded spot | modifier name (default `command`) |
| `savedSpots.recordModifier` | Modifier for the arm-record combo | modifier name / combo (default `command+shift`) |
| `savedSpots.recordKey` | Key that (with `recordModifier`) arms recording of the next slot | key name (default `s`) |
| `savedSpots.armWindowMs` | How long recording stays armed waiting for the slot key | int ≥ 1000 (default `8000`) |
| `savedSpots.spots` | Recorded targets — each `{ key, x, y }` (CG coords). Written by the in-app recorder; edit or clear by hand | array (default `[]`) |
...
AppDelegate (@main)         — wiring, Accessibility permission flow, self-relaunch
  ...
  │                           drag/scroll bookkeeping + grid-jump modal + saved-spot
  │                           record/warp
