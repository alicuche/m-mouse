...
final class GridOverlay {
    ...
    func show(on bounds: CGRect, rows: Int, cols: Int, currentRow: Int, currentCol: Int,
              customLabels: [GridCell: String], spotLabels: [GridCell: String]) {
        ...
        gridView.configure(rows: rows, cols: cols, currentRow: currentRow, currentCol: currentCol,
                           customCells: customLabels, spotCells: spotLabels)
...
private final class GridView: NSView {
    ...
    /// Cells that contain a recorded saved-spot → its warp combo (e.g. "⌘J"),
    /// drawn as a small blue badge under the cell label.
    private var spotCells: [GridCell: String] = [:]
    ...
    func configure(rows: Int, cols: Int, currentRow: Int, currentCol: Int,
                   customCells: [GridCell: String], spotCells: [GridCell: String]) {
        self.rows = rows
        self.cols = cols
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for r in 0..<rows {
            ...
            for c in 0..<cols {
                ...
                // A recorded saved-spot in this cell → its warp combo on a small
                // blue badge directly under the cell label.
                if let spot = spotCells[GridCell(row: r, col: c)] {
                    let spotFont = NSFont.monospacedSystemFont(
                        ofSize: max(9, min(13, cellH * 0.22)), weight: .semibold)
                    let dy = (font.pointSize + spotFont.pointSize) * 0.62 + 3
                    drawSpotBadge(spot, centre: NSPoint(x: cx, y: cy - dy),
                                  font: spotFont, dimmed: dimmed)
                }
            }
        }
    }
    ...
    /// Blue badge marking a recorded saved-spot's warp combo under its cell.
    private static let spotPillFill = NSColor(srgbRed: 0.0, green: 0.48, blue: 1.0, alpha: 1.0)
    ...
    /// Small blue badge (white text) showing a saved-spot's warp combo, drawn
    /// under the cell's main label.
    private func drawSpotBadge(_ text: String, centre: NSPoint, font: NSFont, dimmed: Bool) {
        let textColor = NSColor.white.withAlphaComponent(dimmed ? 0.35 : 1.0)
        let attrs: [NSAttributedString.Key: Any] = [
            .font: font,
            .foregroundColor: textColor,
        ]
        ...
        let pill = NSRect(
            x: centre.x - ts.width / 2 - padX,
            y: centre.y - ts.height / 2 - padY,
            width: ts.width + padX * 2,
            height: ts.height + padY * 2
        )
        let path = NSBezierPath(roundedRect: pill, xRadius: 3, yRadius: 3)
        Self.spotPillFill.withAlphaComponent(dimmed ? 0.25 : 0.95).setFill()
        path.fill()
        NSColor(srgbRed: 0.0, green: 0.20, blue: 0.45, alpha: dimmed ? 0.25 : 0.85).setStroke()
        path.lineWidth = 1
        path.stroke()

        attr.draw(at: NSPoint(x: centre.x - ts.width / 2,
                              y: centre.y - ts.height / 2))
    }
}
